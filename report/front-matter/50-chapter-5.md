...
|   US ID    | US Title                                                                                                                                                                   |    Task Id     | Task Title                                                  | Description                                                                                                                                             |   Assigned To    | Estimation (Hours) | Status |
| :--------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------: | :---------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------ | :--------------: | :----------------: | :----: |
| TS-IAM-001 | Como desarrollador frontend, quiero autenticar usuarios contra el contrato real del backend para permitir el acceso seguro a la plataforma después del registro.           | TS-IAM-001-001 | Align Frontend IAM API With Backend Authentication Contract | Alinear los endpoints, contratos y flujos de autenticación del frontend con los del backend desplegado para que login y register consuman datos reales. | Milenko Cayanchi |         4          |  Done  |
| TS-IAM-001 | Como desarrollador frontend, quiero autenticar usuarios contra el contrato real del backend para permitir el acceso seguro a la plataforma después del registro.           | TS-IAM-001-002 | Refactor Frontend IAM Store                                 | Reestructurar el store de autenticación en el frontend para que gestione sesión, tokens y roles de manera consistente con la API real.                  | Milenko Cayanchi |         3          |  Done  |
| TS-IAM-001 | Como desarrollador frontend, quiero autenticar usuarios contra el contrato real del backend para permitir el acceso seguro a la plataforma después del registro.           | TS-IAM-001-003 | Persist Frontend Auth Session State                         | Implementar persistencia de sesión en el frontend para mantener al usuario autenticado entre recargas de página.                                        |   Zayd Ayasta    |         3          |  Done  |
| TS-IAM-001 | Como desarrollador frontend, quiero autenticar usuarios contra el contrato real del backend para permitir el acceso seguro a la plataforma después del registro.           | TS-IAM-001-004 | Fix JWT Token Generation Compatibility                      | Corregir la compatibilidad en la generación y validación de tokens JWT entre frontend y backend para asegurar sesiones funcionales.                     |   Zayd Ayasta    |         4          |  Done  |
| TS-SUB-001 | Como desarrollador frontend, quiero iniciar el registro de un usuario con suscripción para redirigirlo al checkout y completar su incorporación solo después del pago.     | TS-SUB-001-001 | Implement Pending Registration Aggregate                    | Crear el agregado `PendingRegistration` en el backend para modelar el registro temporal previo al pago.                                                 |   Zayd Ayasta    |         5          |  Done  |
| TS-SUB-001 | Como desarrollador frontend, quiero iniciar el registro de un usuario con suscripción para redirigirlo al checkout y completar su incorporación solo después del pago.     | TS-SUB-001-002 | Implement Start Subscription Registration Command Service   | Implementar el servicio de comando que inicia el registro de suscripción y devuelve la URL de checkout de Stripe.                                       |   Zayd Ayasta    |         5          |  Done  |
| TS-SUB-001 | Como desarrollador frontend, quiero iniciar el registro de un usuario con suscripción para redirigirlo al checkout y completar su incorporación solo después del pago.     | TS-SUB-001-003 | Implement Stripe Checkout Session Creation Service          | Crear el servicio que genera una sesión de Stripe Checkout con el plan seleccionado por el usuario durante el registro.                                 |   Zayd Ayasta    |         4          |  Done  |
| TS-SUB-001 | Como desarrollador frontend, quiero iniciar el registro de un usuario con suscripción para redirigirlo al checkout y completar su incorporación solo después del pago.     | TS-SUB-001-004 | Implement Plan-to-Price Mapping                             | Implementar el mapeo entre los planes de suscripción del frontend y los Price IDs de Stripe para crear la sesión de checkout correcta.                  |   Zayd Ayasta    |         3          |  Done  |
| TS-PFL-001 | Como desarrollador frontend, quiero que la información capturada en el registro se convierta en el perfil final del usuario para completar correctamente su incorporación. | TS-PFL-001-001 | Extend Profiles ACL for Provisioning                        | Extender los servicios ACL de perfiles en el backend para soportar la creación de perfiles durante el flujo de aprovisionamiento post-pago.             | Marcelo Cuadros  |         4          |  Done  |
| TS-PFL-001 | Como desarrollador frontend, quiero que la información capturada en el registro se convierta en el perfil final del usuario para completar correctamente su incorporación. | TS-PFL-001-002 | Implement Restaurant Profile Provisioning                   | Implementar la lógica de creación del perfil de restaurante a partir de los datos capturados en el registro.                                            | Marcelo Cuadros  |         4          |  Done  |
| TS-PFL-001 | Como desarrollador frontend, quiero que la información capturada en el registro se convierta en el perfil final del usuario para completar correctamente su incorporación. | TS-PFL-001-003 | Implement Supplier Profile Provisioning                     | Implementar la lógica de creación del perfil de proveedor a partir de los datos capturados en el registro.                                              | Mathias Sanchez  |         4          |  Done  |
| TS-PFL-001 | Como desarrollador frontend, quiero que la información capturada en el registro se convierta en el perfil final del usuario para completar correctamente su incorporación. | TS-PFL-001-004 | Implement Role-Specific Registration Validation             | Validar que los datos capturados en el registro sean correctos y completos según el rol seleccionado (restaurante o proveedor).                         | Mathias Sanchez  |         3          |  TODO  |
| TS-SUB-003 | Como desarrollador frontend, quiero que el sistema confirme pagos de Stripe y habilite la cuenta del usuario para completar el flujo de registro de extremo a extremo.     | TS-SUB-003-002 | Implement Webhook Signature Validation                      | Implementar la validación de firmas de los webhooks entrantes de Stripe para garantizar que las notificaciones provengan de Stripe.                     |   Zayd Ayasta    |         4          |  Done  |
| TS-SUB-003 | Como desarrollador frontend, quiero que el sistema confirme pagos de Stripe y habilite la cuenta del usuario para completar el flujo de registro de extremo a extremo.     | TS-SUB-003-003 | Implement Checkout Completion Handler                       | Implementar el manejador que procesa el evento `checkout.session.completed` de Stripe para iniciar el aprovisionamiento.                                |   Zayd Ayasta    |         5          |  Done  |
| TS-SUB-003 | Como desarrollador frontend, quiero que el sistema confirme pagos de Stripe y habilite la cuenta del usuario para completar el flujo de registro de extremo a extremo.     | TS-SUB-003-004 | Implement First Payment Provisioning Handler                | Implementar el manejador que, tras confirmar el primer pago exitoso, active la cuenta del usuario y genere los recursos asociados.                      |   Zayd Ayasta    |         6          |  TODO  |
| TS-SUB-003 | Como desarrollador frontend, quiero que el sistema confirme pagos de Stripe y habilite la cuenta del usuario para completar el flujo de registro de extremo a extremo.     | TS-SUB-003-005 | Implement Processed Webhook Event Persistence               | Persistir los eventos de webhook ya procesados para garantizar idempotencia y evitar aprovisionamientos duplicados.                                     |   Zayd Ayasta    |         3          |  Done  |
| TS-SUB-003 | Como desarrollador frontend, quiero que el sistema confirme pagos de Stripe y habilite la cuenta del usuario para completar el flujo de registro de extremo a extremo.     | TS-SUB-003-006 | Implement Re-entrant Provisioning Logic                     | Implementar lógica de aprovisionamiento re-entrante para que, si falla un aprovisionamiento, pueda reintentarse sin duplicar recursos.                  |   Zayd Ayasta    |         4          |  Done  |
| TS-IAM-002 | Como desarrollador frontend, quiero proteger las rutas privadas por autenticación y rol para evitar accesos no autorizados a módulos del sistema.                          | TS-IAM-002-001 | Implement Auth-Aware Global Router Guard                    | Implementar un guarda global de rutas en el frontend que verifique sesión activa antes de permitir el acceso a cualquier ruta privada.                  | Marcelo Cuadros  |         3          |  Done  |
| TS-IAM-002 | Como desarrollador frontend, quiero proteger las rutas privadas por autenticación y rol para evitar accesos no autorizados a módulos del sistema.                          | TS-IAM-002-002 | Remove Invalid Role Auto-Assignment Logic                   | Eliminar la lógica obsoleta de asignación automática de roles en el frontend para que el rol sea provisto exclusivamente por el backend.                | Marcelo Cuadros  |         2          |  Done  |
| TS-IAM-002 | Como desarrollador frontend, quiero proteger las rutas privadas por autenticación y rol para evitar accesos no autorizados a módulos del sistema.                          | TS-IAM-002-003 | Implement Role Scope Redirect Rules                         | Implementar reglas de redirección que envíen al usuario al dashboard correcto según el rol asignado por el backend.                                     | Milenko Cayanchi |         3          |  Done  |
| TS-IAM-002 | Como desarrollador frontend, quiero proteger las rutas privadas por autenticación y rol para evitar accesos no autorizados a módulos del sistema.                          | TS-IAM-002-004 | Normalize Auxiliary Authentication Guard                    | Normalizar los guardas auxiliares de autenticación para que todos sigan el mismo patrón y no existan duplicados ni conflictos.                          | Marcelo Cuadros  |         3          |  Done  |
